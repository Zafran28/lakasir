...
use App\Filament\Tenant\Pages\Finance;
...
class TenantPanelProvider extends PanelProvider
{
    ...
    private function initializeTenantPanel(Panel $panel, string $url): void
    {
        try {
            ...
            $tenant = Tenant::whereHas(
                'domains',
                fn ($query) => $query->where('domain', $url)
            )->first();
            ...
            if ($tenant) {
                ...
                config([
                    'cache.prefix' => $subdomain . '_'
                ]);

                app(DatabaseTenancyBootstrapper::class)
                    ->bootstrap($tenant);

                tenant()->run(
                    fn () => $this->configureTenantBrand($panel)
                );
    ...
    private function generateNavigationItem(
        string $resource,
        ?string $feature = null,
        ?array $activeWhen = []
    ): NavigationItem {
        ...
        $canAccess = $feature
            ? feature($feature) && $resource::canAccess()
            : $resource::canAccess();
        ...
        /**
         * =========================
         * PAGE ACTIVE DETECTION
         * =========================
         */
        if ($instance instanceof Page) {

            if (method_exists($resource, 'getRouteName')) {
                $active = Route::currentRouteName() === $resource::getRouteName();
            }
        }

        /**
         * =========================
         * RESOURCE ACTIVE DETECTION
         * =========================
         */
        if ($instance instanceof Resource) {

            $active = Str::of(Route::currentRouteName())
                ->contains($resource::getRouteBaseName());
        }

        /**
         * =========================
         * CUSTOM ACTIVE OVERRIDE
         * =========================
         */
        if (!empty($activeWhen)) {
            ...
            foreach ($activeWhen as $class) {
                if (method_exists($class, 'getRouteName')) {
                    $routes[] = $class::getRouteName();
                }
            }

            if (method_exists($resource, 'getRouteName')) {
                $routes[] = $resource::getRouteName();
            }

            $active = in_array(Route::currentRouteName(), $routes);
        }

        /**
         * =========================
         * SAFE LABEL (FIX ERROR KAMU)
         * =========================
         */
        $label = method_exists($resource, 'getLabel')
            ? $resource::getLabel()
            : (method_exists($resource, 'getNavigationLabel')
         ...
         * =========================
         */
        $url = method_exists($resource, 'getUrl')
            ? $resource::getUrl()
            : '#';
