    ...
    public function generate(array $data)
    {
        ...
        foreach ($sellings as $selling) {
            ...
            foreach ($selling->sellingDetails as $detail) {
                ...
                // skip product yang sudah dihapus
                if (!$detail->product) {
                    continue;
                }
                ...
                $totalAfterDiscountPerSelling += (
                    $detail->price - ($detail->discount_price ?? 0)
                );

                $totalNetProfitPerSelling += (
                    ($detail->price - $detail->cost)
                ...
                $reports[] = [
                    ...
                    'selling_price' => $this->formatCurrency(
                        $detail->qty > 0
                            ? ($detail->price / $detail->qty)
                            : 0
                    ),

                    'selling' => $this->formatCurrency(
                        $detail->price - ($detail->discount_price ?? 0)
                    ),

                    'discount_price' => $this->formatCurrency(
                        $detail->discount_price ?? 0
                    ),

                    'initial_price' => $this->formatCurrency(
                        $detail->qty > 0
                            ? ($detail->cost / $detail->qty)
                            : 0
                    ),
                    ...
                    'total_after_discount' => $this->formatCurrency(
                        $detail->price - ($detail->discount_price ?? 0)
                    ),

                    'net_profit' => $this->formatCurrency(
                        (
                            $detail->price
                            - ($detail->discount_price ?? 0)
                        ) - $detail->cost
                    ...
                    'gross_profit' => $this->formatCurrency(
                        $detail->price - $detail->cost
                    ),
                ];
            }
            ...
            $totalNetProfitBeforeDiscountSelling +=
                $totalNetProfitPerSelling;

            $totalNetProfitAfterDiscountSelling += (
                $totalNetProfitPerSelling
                - ($selling->discount_price ?? 0)
            );
        ...
        $footer = [
            ...
            'total_net' => $this->formatCurrency(
                $totalNet - $totalDiscount
            ),

            'total_net_price_after_discount_per_item' =>
                $this->formatCurrency($totalNet),

            'total_net_price_after_discount_selling' =>
                $this->formatCurrency(
                    $totalNet - $totalDiscount
                ),

            'total_discount' => $this->formatCurrency(
                $totalDiscount
            ),

            'total_discount_per_item' => $this->formatCurrency(
                $totalDiscountPerItem
            ),

            'total_gross_profit' => $this->formatCurrency(
            ...
            'total_net_profit_after_discount_selling' =>
                $this->formatCurrency(
                    $totalNet - $totalDiscount - $totalCost
                ),
